...
(async () => {
  ...
  try {
    ...
    for (const [name, variants] of [
      ...
      await ownerCall('/menu/items', {
        ...
        variants: variants.map((name) => ({
          name,
          channels: [{ channelCode: 'COUNTER', price: '100', available: true }],
        })),
      });
    chrome = spawn(
      process.env.CHROME_BINARY,
    ...
    assert.ok(
      await a.read(
        "[...document.querySelectorAll('.kds-action')].every(b=>b.getBoundingClientRect().height>=60)",
      ),
    );
    shot = await a.send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(
