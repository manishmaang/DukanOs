...
See [Orders](modules/orders.md) for transaction, idempotency and FIFO read contracts. DRAFT is a local cart; confirmation persists QUEUED, names/prices/tax snapshots, confirmed actor and append-only DRAFT→QUEUED history. UUID identifies orders; date-scoped tokens reset at restaurant midnight via PostgreSQL allocation. No payment is implied or recorded. Confirmed sale data cannot be edited; Orders owns the implemented audited Kitchen lifecycle commands. KITCHEN uses dedicated operational reads and has no generic financial order-read or creation permission.
