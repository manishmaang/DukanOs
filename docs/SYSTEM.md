...
Counter confirmation and KDS use only local API/PostgreSQL and locally served assets. Installation initially needs internet access to download packages/images.
...
Next proposed milestone: a focused Dispatch handoff for READY → COMPLETED. Do not begin automatically. Later phases: Payments, Amendments, Customers, Credit, Reports, Integrations. Menu modifiers remain deferred.
...
See [Orders](modules/orders.md) for transaction, idempotency and FIFO read contracts. DRAFT is a local cart; confirmation persists QUEUED, names/prices/tax snapshots, confirmed actor and append-only DRAFT→QUEUED history. UUID identifies orders; date-scoped tokens reset at restaurant midnight via PostgreSQL allocation. No payment is implied or recorded. Confirmed sale data cannot be edited; Orders owns the implemented audited Kitchen lifecycle commands. KITCHEN has orders.read but no creation permission.

## Kitchen

Order View separates QUEUED/PREPARING, marks the oldest cross-date queued order NEXT and permits FIFO START only; READY leaves active Kitchen. Production View separates To start/In preparation and aggregates immutable item/variant snapshots with every source token, line quantity and independent instruction preserved. No financial data appears in Kitchen projections. Preparation status is whole-order, not per item. Two-second local polling plus action/focus/reconnect refetch synchronizes devices; failure hides stale actions. Migration 009 enables audited history-driven START/READY, retaining financial/item immutability and existing data. See [Kitchen](modules/kitchen.md) for contracts, concurrency and limits.
