...
A frontend OperationalAudio coordinator consumes those existing projections, never mutates domain records and uses one lazily gesture-unlocked Web Audio context. It owns workspace/capability scope, coalescing, repeat deadlines and non-overlapping patterns; localStorage retains only the browser mute preference. The shared alert hook also polls when mounted in a running background tab to reconcile resolutions. Browser/OS throttling and suspension remain outside application guarantees. No backend, schema, provider or dependency changes are needed for sound.
