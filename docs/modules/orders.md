...
POS Confirm Order opens a compact server-priced review. POST /api/orders/counter/quote uses the same input validation, current Counter availability/price/tax resolution and bill lock as confirmation, but allocates no bill, order, token or payment. It requires orders.create + bills.read + payments.read and returns roundTotal, existingDue and amountDue. Existing due plus the new round is the payable balance; collection is bill-level, not per token.
