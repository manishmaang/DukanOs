...
Completed verification on 2026-09-22: `npm run check` (8 API unit tests and 7 frontend/helper tests), all 57 PostgreSQL integration tests, and local Chromium browser regression passed. Browser checks covered Soya Chaap Half ×1 + Full ×1 / Extra spicy in one Add, another line's quantity/removal/re-addition, consecutive tokens, double-click, another-session sold-out rejection, lost response and reload/retry returning the same token. Multi-selection coverage additionally verifies three portions, zero exclusions, note overrides, single-portion dishes, cart preservation and long-name four-portion dialogs at 1440×900, 1366×768 and 768×1024. External browser requests were blocked. This verifies local application dependencies, not a physical router-disconnection exercise. Migration 008 was applied locally with before/after equality checks for users, assignments, audits, catalog and image records plus file hashes; all were preserved.
...
A shared kitchen instruction applies to every selected portion. Optional Add note opens a per-portion override; Use shared note removes the override. An explicitly empty override means no instruction for that portion. Notes retain the existing 500-character bound and plain-text semantics. Different variants/instructions remain separate; matching lines merge under the existing rule. The batch is applied atomically in browser state: exceeding a cart/line limit leaves the entire cart unchanged. Cart lines remain editable separately for multiple customizations of the same portion.
